...
# experiment 2  将编码器模块中每块增加一层卷积层，并在下采样层最后添加dropout=0.5 add weight-decay, RandomGammaTransform, RandomBilateralFilter, RandomNoise
CUDA_VISIBLE_DEVICES=0,1 python train.py --learn-rate 0.001 --weight-decay 0.001 --epochs 1000 --batch-size 20 --test-batch-size 20 --base-size 400 --crop-size 400 --gpu-ids 0,1 --checkname unet --eval-interval 1 --dataset rssrai2019