...
def crop_tiff_image(filename, source_dir, crop_size, target_dir, postfix):
    ...
    if rows % crop_rows != 0 or cols % crop_cols != 0:
    ...
    for i in range(rows_num):
        for j in range(cols_num):
            ...
            if len(img_np.shape) == 2:
                crop_img_np = img_np[i * crop_rows:i * crop_rows + crop_rows, j * crop_cols:j * crop_cols + crop_cols]
    ...
    for root, dirs, files in os.walk(source_dir):
        for file in files:
            if os.path.splitext(file)[1] == postfix:
                ...
                crop_tiff_image(file, root, crop_size, target_dir, postfix)
                n += 1
    print('crop finished')
